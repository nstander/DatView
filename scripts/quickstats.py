...
parser=argparse.ArgumentParser(description='Show the min, min(after invalid masked), mean (after invalid masked), max of each column.')
...
for col in model.cols:
    print (col,np.min(model.data[col]), model.fieldmin(col), np.mean(model.data[col][model.data[col]!= model.cfg.nullvalue]),np.max(model.data[col]),sep="\t")
